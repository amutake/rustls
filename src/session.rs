use cipher::{self, MessageDecrypter, MessageEncrypter};
use error::TLSError;
use hash_hs;
use key;
use key_schedule::{KeySchedule, Protocol, SecretKind};
use msgs::base::Payload;
use msgs::codec::Codec;
use msgs::deframer::MessageDeframer;
use msgs::enums::KeyUpdateRequest;
use msgs::enums::{AlertDescription, AlertLevel, ContentType, ProtocolVersion};
...
pub struct SessionCommon {
    ...
    pub sendable_msgs: VecDeque<Message>,
...
impl SessionCommon {
    ...
    pub fn set_message_encrypter(&mut self, cipher: Box<MessageEncrypter>) {
        self.message_encrypter = cipher;
        self.write_seq = 0;
        self.we_encrypting = true;
    }

    pub fn set_message_decrypter(&mut self, cipher: Box<MessageDecrypter>) {
