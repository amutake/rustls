...
impl KeySchedule {
    ...
    pub fn quic_initial() -> KeySchedule {
        ...
        const INITIAL_SALT: &[u8] = &[
            0x9c, 0x10, 0x8f, 0x98, 0x52, 0x0a, 0x5c, 0x5c, 0x32, 0x96, 0x8e, 0x95, 0x0e, 0x8a,
...
fn _hkdf_expand_label_vec(
    ...
    let mut v = Vec::new();
    v.resize(len, 0u8);
    _hkdf_expand_label(&mut v, secret, label_prefix, label, context);
    v
}

fn _hkdf_expand_label(
    output: &mut [u8],
    secret: &hmac::SigningKey,
    label_prefix: &[u8],
    label: &[u8],
...
impl TrafficKey {
    pub fn new(
        ...
        let key = _hkdf_expand_label_vec(
            &hmac::SigningKey::new(hash, secret),
            protocol.prefix(),
            b"key",
            &[],
            key_len,
        );
        let iv = _hkdf_expand_label_vec(
            &hmac::SigningKey::new(hash, secret),
            protocol.prefix(),
            b"iv",
            &[],
...
#[cfg(test)]
mod test {
    use super::{KeySchedule, Protocol, SecretKind, TrafficKey};
    use ring::digest;
    ...
    fn test_vectors() {
        ...
        assert_eq!(traffic.iv, client_hts_iv.to_vec());
        ...
        assert_eq!(traffic.iv, server_hts_iv.to_vec());
        ...
        assert_eq!(traffic.iv, client_ats_iv.to_vec());
        ...
        assert_eq!(traffic.iv, server_ats_iv.to_vec());
    }
}
